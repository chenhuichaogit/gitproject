...
var config = {
  ...
  colors: ['#008703', '#f7a35c', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
  colors2: ['#58b5fd', '#fd8989'],
...
function fillSeriesColor(series, config) {
  ...
  return series.map(function (item) {
    if (!item.color) {
      item.color = config.colors2[index];
...
function drawRadarLabel(angleList, radius, centerPosition, opts, config, context) {
  ...
  context.beginPath();
  context.setFontSize(config.fontSize);
  context.setFillStyle(radarOption.labelColor || '#585858');
  angleList.forEach(function (angle, index) {
    ...
    if (util.approximatelyEqual(pos.x, 0)) {
      startX -= measureText(opts.categories[index] || '') / 6;
    } else if (pos.x < 0) {
      startX -= measureText(opts.categories[index] || '') / 4;
    }

    var filltext = opts.categories[index].split(',')
    //console.log('filltext=====' + filltext[0])
    context.setTextAlign('center')
    context.fillText(filltext[0] + filltext[1] || '', startX, startY + config.fontSize / 2);
    context.fillText(filltext[2] || '', startX, startY + config.fontSize * 2);
  });
  context.stroke();
  context.closePath();
}
...
function drawXAxis(categories, opts, config, context) {
  ...
  if (config._xAxisTextAngle_ === 0) {
    ...
    categories.forEach(function (item, index) {
      context.save();
      context.beginPath();
      context.setFontSize(10)//(config.fontSize);
      context.setFillStyle(opts.xAxis.fontColor || '#585858');
      ...
      //console.log('test==========' + itemt[0])

      context.fillText(itemt[0], xAxisPoints[index] + offset, startY + config.fontSize + 5);
      //context.fillText(itemt[1], xAxisPoints[index] + offset + 30, startY + config.fontSize + 10);
      context.closePath();
      context.stroke();
      context.restore();
    });
...
function drawLegend(series, opts, config, context) {
  ...
  legendList.forEach(function (itemList, listIndex) {
    ...
    var startX = padding;// (opts.width - width) / 2 + padding;
...
function drawRadarDataPoints(series, opts, config, context) {
  ...
  var radius = Math.min(centerPosition.x - (getMaxTextListLength(opts.categories) / 2 + config.radarLabelTextMargin), centerPosition.y - config.radarLabelTextMargin);
  ...
  var _loop = function _loop(i) {
    ...
    // context.beginPath();
    // context.setLineWidth(2);
    // context.setStrokeStyle(radarOption.gridColor || "#000000");
    ...
    coordinateAngle.forEach(function (angle, index) {
      ...
      if (radius >= 0) {
        var t = Math.sqrt(Math.pow((centerPosition.x - pos.x), 2) + Math.pow((centerPosition.y - pos.y), 2))
        context.arc(centerPosition.x, centerPosition.y, t, 0, 2 * (radius * index / 360) * Math.PI);
      }
    });
    context.setFillStyle('rgba(0,100,180,0.1)')
    context.fill();
    context.closePath();
